...
$PItem = 10; //一頁顯示的筆數
$mysql->table = 'parts_data'; //指定table
//mysql_num_rows($mysql->select('','parts_id'))  此段是抓table值 的數量
//$_SERVER['PHP_SELF'].'?' 抓目前檔案的名稱
$MyPage = GPage($PItem,$Page,mysql_num_rows($mysql->select('','parts_id')),$_SERVER['PHP_SELF'].'?',''); //抓頁數的Function 如果需要傳另外的值,可用最後一個欄位
...
?> 
        ...
        <div class="maincontent">
        	<div class="contentinner">
            	<h4 class="widgettitle"><span class="iconsweets-dropbox"></span> 零件列表<a href="parts_new.php" title="編輯" class="btn btn-primary" style="float:right" ><span class="icon-plus"></span>新增</a></h4>
                <table class="table table-bordered">
					...
					jQuery(document).ready(function(){
						jQuery('#deleter<?=$row['parts_id'];?>').click(function(){
						   var conf = confirm('是否確定刪除?');
						   if(conf)
							  window.location.href='parts_del.php?pid=<?=$row['parts_id'];?>';
						   return false;
						}); 
					});   
					</script>	
                        <tr>
                            ...
                            <a href="parts_view.php?pid=<?=$row['parts_id'];?>" title="編輯" class="btn" ><span class="icon-edit"></span>編輯</a>
                            &nbsp;
                            <a title="刪除" id="deleter<?=$row['parts_id'];?>" style="cursor: pointer;" class="btn"><span class="icon-trash"></span>刪除</a>
                            </td>
                        </tr>
                    <? }?>    
                    </tbody>
