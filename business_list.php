...
$MyPage = GPage($PItem,$Page,mysql_num_rows($mysql->select('','BID')),$_SERVER['PHP_SELF'].'?',''); //抓頁數的Function 如果需要傳另外的值,可用最後一個欄位
$rs = $mysql->select("ORDER BY ATime DESC LIMIT ".$MyPage['Str'].','.$PItem,'*,(SELECT user_name FROM(member_data) WHERE member_data.member_id=Business_data.member_id) AS Name');
?>
        ...
        <table class="table table-bordered">
          ...
          <tbody>
            ...
            <tr id="tr_<?=$row['BID']?>" <? if ($row['cancel']==1){?> style=" background-color:#FB7C66;"<? }?>>
              ...
              <td><?=$row['Name']; ?></td>
              <td><?=$info['IName']; ?></td>
              <td><?php if($row['Checked']){ echo $row['End'] ? '<span style="color:#6C0">Shipment is completed</span>' : 'No shipments'; } else { echo '<span style="color:#F00">Processing</span>'; }?></td>
              ...
              <td class="centeralign"><?php if($row['Checked']){?>
                <a href="javascript:;" onClick="opendialog1('<?=$row['BID']?>');" title="Details" class="btn" ><span class="icon-pencil"></span>&nbsp;Details</a> <a href="business_print.php?BID=<?=$row['BID']?>" title="Print" target="_blank" class="btn" ><span class="icon-print"></span>&nbsp;Print </a>
                <?php } else {?>
